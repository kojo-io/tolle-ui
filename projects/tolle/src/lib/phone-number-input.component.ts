...
export class PhoneNumberInputComponent implements ControlValueAccessor {
    ...
    dataType = input<'NumberOnly' | 'FullObject'>('FullObject');
    mask = input('(000) 000-0000');
    class = input('');
    ...
    constructor() {
        // Initializing will happen in writeValue or via defaultCountryCode if no value provided
    }
        ...
        if (value) {
            if (typeof value === 'object' && value.number) {
                ...
                try {
                    ...
                    if (parsed) {
                        this.selectedIso = (parsed.country as string) || this.defaultCountryCode();
                    } else {
