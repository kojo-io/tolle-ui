import {
  Component, input, OnInit, forwardRef, output, signal, computed, effect
} from '@angular/core';

import { ControlValueAccessor, NG_VALUE_ACCESSOR, FormsModule } from '@angular/forms';
import {
  addMonths, subMonths, startOfMonth, endOfMonth,
  startOfWeek, endOfWeek, eachDayOfInterval, isSameMonth,
  isSameDay, isToday, setMonth, setYear, addYears, subYears,
  isBefore, startOfDay, format
} from 'date-fns';
import { cn } from './utils/cn';

export type CalendarMode = 'date' | 'month' | 'year';

@Component({
  selector: 'tolle-calendar',
  standalone: true,
  imports: [FormsModule],
  providers: [
    {
      provide: NG_VALUE_ACCESSOR,
      useExisting: forwardRef(() => CalendarComponent),
      multi: true
    }
  ],
  template: `
    <div [class]="cn('p-3 border rounded-md bg-background text-popover-foreground shadow-sm inline-block min-w-fit', class())">
      <!-- Header with Navigation -->
      <div class="flex items-center justify-between pt-1 pb-4 gap-2">
        <!-- View Selector -->
        <div class="flex items-center gap-1">
          @if (mode() !== 'year') {
            <button
              type="button"
              (click)="setView('month')"
            [class]="cn(
              'text-sm font-semibold px-2 py-1 rounded transition-colors',
              currentView() === 'month' ? 'bg-secondary text-secondary-foreground' : 'hover:bg-accent hover:text-accent-foreground'
            )">
              {{ formatMonthYear(viewDate(), 'month') }}
            </button>
          }
    
          <button
            type="button"
            (click)="setView('year')"
            [class]="cn(
              'text-sm font-semibold px-2 py-1 rounded transition-colors',
              currentView() === 'year' ? 'bg-secondary text-secondary-foreground' : 'hover:bg-accent hover:text-accent-foreground'
            )">
            {{ formatMonthYear(viewDate(), 'year') }}
          </button>
        </div>
    
        <!-- Navigation Buttons -->
        <div class="flex items-center space-x-1">
          <button type="button" (click)="prev()" [class]="navBtnClass">
            <i class="ri-arrow-left-s-line text-lg"></i>
          </button>
          <button type="button" (click)="next()" [class]="navBtnClass">
            <i class="ri-arrow-right-s-line text-lg"></i>
          </button>
        </div>
      </div>
    
      <!-- DATE MODE -->
      @if (currentView() === 'date' && mode() === 'date') {
        <div class="space-y-2 animate-in fade-in zoom-in-95 duration-200">
          <div class="grid grid-cols-7 gap-1 w-full">
            @for (day of weekDays; track day) {
              <span class="text-[0.8rem] text-muted-foreground font-normal text-center w-9">
                {{ day }}
              </span>
            }
          </div>
          <div class="grid grid-cols-7 gap-1 w-full">
            @for (date of daysInMonth(); track date.getTime()) {
              <button
                type="button"
                (click)="selectDate(date)"
                [disabled]="isDateDisabled(date)"
                [class]="getDayClass(date)"
                >
                {{ formatDate(date, 'day') }}
              </button>
            }
          </div>
        </div>
      }
    
      <!-- MONTH SELECTOR (for date mode and month mode) -->
      @if ((currentView() === 'month')) {
        <div
          class="grid grid-cols-3 gap-2 w-64 animate-in fade-in zoom-in-95 duration-200">
          @for (month of months; track month; let i = $index) {
            <button
              type="button"
              (click)="selectMonth(i)"
              [class]="getMonthClass(i)"
              >
              {{ month }}
            </button>
          }
        </div>
      }
    
      <!-- YEAR SELECTOR (for date mode and year mode) -->
      @if ((currentView() === 'year') ) {
        <div
          class="grid grid-cols-4 gap-2 w-64 animate-in fade-in zoom-in-95 duration-200">
          @for (year of years(); track year) {
            <button
              type="button"
              (click)="selectYear(year)"
              [class]="getYearClass(year)"
              >
              {{ year }}
            </button>
          }
        </div>
      }
    
      <!-- Quick Actions -->
      @if (showQuickActions()) {
        <div class="border-t pt-3 mt-3">
          <div class="flex items-center justify-between gap-2">
            <button
              type="button"
              (click)="selectToday()"
              [class]="quickActionBtnClass"
              [disabled]="isTodayDisabled()"
              >
              Today
            </button>
            <button
              type="button"
              (click)="clear()"
              [class]="quickActionBtnClass"
              >
              Clear
            </button>
          </div>
        </div>
      }
    </div>
  `
})
export class CalendarComponent implements OnInit, ControlValueAccessor {
  class = input('');
  mode = input<CalendarMode>('date');
  disablePastDates = input(false);
  showQuickActions = input(true);
  minDate = input<Date>();
  maxDate = input<Date>();
  formatMonthFn = input<(date: Date) => string>();
  formatYearFn = input<(date: Date) => string>();
  formatDateFn = input<(date: Date) => string>();

  dateSelect = output<Date | null>();

  currentView = signal<'date' | 'month' | 'year'>('date');
  viewDate = signal(new Date());
  selectedDate = signal<Date | null>(null);

  weekDays = ['Su', 'Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa'];
  daysInMonth = signal<Date[]>([]);
  months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
  years = signal<number[]>([]);
  yearRangeStart = signal<number>(new Date().getFullYear() - 6);

  navBtnClass = cn('h-7 w-7 bg-transparent p-0 opacity-50 hover:opacity-100 border border-input rounded-md flex items-center justify-center hover:bg-accent hover:text-accent-foreground transition-all');
  quickActionBtnClass = cn('px-3 py-1.5 text-sm rounded hover:bg-accent text-muted-foreground hover:text-foreground transition-colors');

  onTouched: () => void = () => { };
  onChange: (value: any) => void = () => { };

  protected cn = cn;

  constructor() {
    effect(() => {
      this.generateDays();
    }, { allowSignalWrites: true });

    effect(() => {
      this.generateYears();
    }, { allowSignalWrites: true });
  }

  ngOnInit() {
    // Initialize based on mode
    if (this.mode() === 'month') {
      this.currentView.set('month');
    } else if (this.mode() === 'year') {
      this.currentView.set('year');
    }
  }

  // Format helpers
  formatMonthYear(date: Date, type: 'month' | 'year'): string {
    const fn = type === 'month' ? this.formatMonthFn() : this.formatYearFn();
    if (fn) return fn(date);
    return type === 'month' ? format(date, 'MMMM') : format(date, 'yyyy');
  }

  formatDate(date: Date, type: 'day' | 'month' | 'year'): string {
    const fn = this.formatDateFn();
    if (type === 'day' && fn) {
      return fn(date);
    }
    return format(date, type === 'day' ? 'd' : type === 'month' ? 'MMM' : 'yyyy');
  }

  generateDays() {
    if (this.mode() !== 'date') return;

    const start = startOfWeek(startOfMonth(this.viewDate()));
    const end = endOfWeek(endOfMonth(this.viewDate()));
    this.daysInMonth.set(eachDayOfInterval({ start, end }));
  }

  generateYears() {
    const currentYear = this.viewDate().getFullYear();

    if (this.mode() === 'year') {
      // For year picker, show a 12-year grid
      this.years.set(Array.from({ length: 12 }, (_, i) => this.yearRangeStart() + i));
    } else {
      // For date mode year selector, show 16 years centered on current
      this.years.set(Array.from({ length: 16 }, (_, i) => currentYear - 6 + i));
    }
  }

  setView(view: 'date' | 'month' | 'year') {
    this.currentView.set(view);
    if (view === 'year') {
      this.generateYears();
    }
  }

  prev() {
    const currentMode = this.mode();
    const currentView = this.currentView();
    const viewDate = this.viewDate();

    if (currentMode === 'date') {
      if (currentView === 'date') {
        this.viewDate.set(subMonths(viewDate, 1));
      } else if (currentView === 'year') {
        this.viewDate.set(subYears(viewDate, 16));
      } else if (currentView === 'month') {
        this.viewDate.set(subYears(viewDate, 1));
      }
    } else if (currentMode === 'month') {
      this.viewDate.set(subYears(viewDate, 1));
    } else if (currentMode === 'year') {
      this.yearRangeStart.update((v: number) => v - 12);
    }
  }

  next() {
    const currentMode = this.mode();
    const currentView = this.currentView();
    const viewDate = this.viewDate();

    if (currentMode === 'date') {
      if (currentView === 'date') {
        this.viewDate.set(addMonths(viewDate, 1));
      } else if (currentView === 'year') {
        this.viewDate.set(addYears(viewDate, 16));
      } else if (currentView === 'month') {
        this.viewDate.set(addYears(viewDate, 1));
      }
    } else if (currentMode === 'month') {
      this.viewDate.set(addYears(viewDate, 1));
    } else if (currentMode === 'year') {
      this.yearRangeStart.update((v: number) => v + 12);
    }
  }

  selectDate(date: Date) {
    if (this.isDateDisabled(date)) return;

    this.selectedDate.set(date);
    this.onChange(date);
    this.onTouched();
    this.dateSelect.emit(date);
  }

  selectMonth(monthIndex: number) {
    const currentViewDate = this.viewDate();
    if (this.mode() === 'date') {
      this.viewDate.set(setMonth(currentViewDate, monthIndex));
      this.currentView.set('date');
    } else if (this.mode() === 'month') {
      const newDate = setMonth(currentViewDate, monthIndex);
      this.viewDate.set(newDate);
      this.selectedDate.set(newDate);
      this.onChange(newDate);
      this.onTouched();
      this.dateSelect.emit(newDate);
    }
  }

  selectYear(year: number) {
    const currentViewDate = this.viewDate();
    if (this.mode() === 'date') {
      this.viewDate.set(setYear(currentViewDate, year));
      this.currentView.set('date');
    } else if (this.mode() === 'year' || this.mode() === 'month') {
      const newDate = setYear(currentViewDate, year);
      this.viewDate.set(newDate);
      this.selectedDate.set(newDate);
      this.onChange(newDate);
      this.onTouched();
      this.dateSelect.emit(newDate);
    }
  }

  getDayClass(date: Date) {
    const selected = this.selectedDate();
    const isSelected = selected && isSameDay(date, selected);
    const isTodayDate = isToday(date);
    const isOutside = !isSameMonth(date, this.viewDate());
    const isDisabled = this.isDateDisabled(date);

    return cn(
      'h-9 w-9 p-0 font-normal text-sm rounded-md transition-all flex items-center justify-center',
      !isSelected && !isDisabled && 'hover:bg-accent hover:text-accent-foreground',
      isSelected && 'bg-primary text-primary-foreground hover:bg-primary hover:text-primary-foreground',
      !isSelected && isTodayDate && 'bg-accent text-accent-foreground',
      (isOutside || isDisabled) && 'text-muted-foreground opacity-50',
      isDisabled && 'cursor-not-allowed'
    );
  }

  getMonthClass(monthIndex: number) {
    const selected = this.selectedDate();
    const isSelected = selected &&
      selected.getMonth() === monthIndex &&
      selected.getFullYear() === this.viewDate().getFullYear();
    const isCurrent = new Date().getMonth() === monthIndex &&
      new Date().getFullYear() === this.viewDate().getFullYear();

    return cn(
      'text-sm py-2.5 rounded-md transition-colors',
      isSelected ? 'bg-primary text-primary-foreground hover:bg-primary' :
        isCurrent ? 'border border-primary/30 text-primary' :
          'hover:bg-accent hover:text-accent-foreground'
    );
  }

  getYearClass(year: number) {
    const selected = this.selectedDate();
    const isSelected = selected && selected.getFullYear() === year;
    const isCurrent = new Date().getFullYear() === year;

    return cn(
      'text-sm py-2 rounded-md transition-colors',
      isSelected ? 'bg-primary text-primary-foreground hover:bg-primary' :
        isCurrent ? 'border border-primary/30 text-primary' :
          'hover:bg-accent hover:text-accent-foreground'
    );
  }

  isDateDisabled(date: Date): boolean {
    if (this.disablePastDates() && isBefore(date, startOfDay(new Date()))) {
      return true;
    }
    const min = this.minDate();
    if (min && isBefore(date, min)) {
      return true;
    }
    const max = this.maxDate();
    return !!(max && isBefore(max, date));
  }

  isTodayDisabled(): boolean {
    return this.isDateDisabled(new Date());
  }

  selectToday() {
    if (!this.isTodayDisabled()) {
      this.selectDate(new Date());
    }
  }

  clear() {
    this.selectedDate.set(null);
    this.onChange(null);
    this.onTouched();
    this.dateSelect.emit(null);
  }

  // CVA Implementation
  writeValue(obj: any): void {
    if (obj) {
      const date = new Date(obj);
      if (!isNaN(date.getTime())) {
        this.selectedDate.set(date);
        this.viewDate.set(date);
      }
    }
  }
  registerOnChange(fn: any): void { this.onChange = fn; }
  registerOnTouched(fn: any): void { this.onTouched = fn; }
}
