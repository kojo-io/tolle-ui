...
const llmsCtxPath = path.join(publicDir, 'llms-ctx.txt');
...
- [Full Context Documentation](llms-ctx.txt): For AI agents to ingest the entire library.
...
function renderComponentGroup(key: string, group: any): string {
    ...
    group.components.forEach((comp: any) => {
        if (comp.name.includes('Overview')) {
            md += `### Overview & Installation\n`;
            if (comp.installation) {
                md += `\`\`\`typescript\n${comp.installation}\n\`\`\`\n\n`;
            }
        ...
        if (comp.name.includes('Interactive') || comp.name.includes('Examples')) {
            md += `### Usage Examples\n`;
            Object.keys(comp.examples).forEach(ex => {
        ...
        if (comp.name.includes('Api')) {
            md += `### API Reference\n`;
            Object.keys(comp.props).forEach(propGroup => {
